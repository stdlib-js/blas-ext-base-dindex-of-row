...
tape( 'the function has an arity of 9', function test( t ) {
	t.strictEqual( dindexOfRow.length, 9, 'returns expected value' );
	t.end();
});
...
tape( 'the function throws an error if provided a first argument which is not a valid order', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dindexOfRow( value, data.M, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
		};
	}
});
...
tape( 'the function throws an error if provided a fifth argument which is not a valid `LDA` value (row-major)', function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			dindexOfRow( data.order, data.M, data.N, new Float64Array( data.A ), value, new Float64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
		};
	}
});
...
tape( 'the function returns an invalid index when M is less than or equal to zero', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = dindexOfRow( data.order, 0, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
...
tape( 'the function returns an invalid index when N is less than or equal to zero', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = dindexOfRow( data.order, data.M, 0, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
...
tape( 'the function returns the index of the first row matching a search vector (row-major)', function test( t ) {
	...
	data = ROW_MAJOR_DATA;
	out = dindexOfRow( data.order, data.M, data.N, new Float64Array( data.A ), data.LDA, new Float64Array( data.x ), data.strideX, new Uint8Array( data.M ), 1 );
